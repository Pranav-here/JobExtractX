...
class T5Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # T5 expects inputs with a "prefix" that defines the task
        # Assuming this is a generic text-to-text task, adjust if needed
        source_text = f"transform: {source_text}"
...
train_dataset = T5Dataset(train_data, tokenizer, max_source_length=max_source_length, max_target_length=max_target_length)
val_dataset = T5Dataset(val_data, tokenizer, max_source_length=max_source_length, max_target_length=max_target_length)

# Set up training arguments - RTX 6000 has 24GB VRAM
# Using gradient accumulation for handling long sequences
training_args = TrainingArguments(
    output_dir="./t5_model_output",
    num_train_epochs=3,
...
def generate_text(input_text, max_length=100):
    inputs = tokenizer(f"transform: {input_text}", return_tensors="pt", truncation=True, max_length=max_source_length)
