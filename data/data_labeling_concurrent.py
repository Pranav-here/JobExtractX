...
def process_jobs_concurrently(jobs):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        with tqdm(total=len(futures), desc="Processing jobs") as pbar:
            for future in concurrent.futures.as_completed(futures):
                job = futures[future]
                try:
                    future.result()  # We don't need to handle the result here as it's already processed
                except Exception as e:
                    print(f"Job {job['_id']} generated an exception: {e}")
                finally:
                    pbar.update(1)  # Update the progress bar for each completed future

# Convert the cursor to a list and process jobs concurrently
jobs = list(to_label_job_data)
process_jobs_concurrently(jobs)
